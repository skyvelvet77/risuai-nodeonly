...
- ✨ **NodeOnly 전용 편의기능이 포함되어 있습니다.** 채팅별 프리셋 바인딩, 삽화 갤러리, 삽화 일괄 압축, 업데이트 자동 알림 등 원본에 없는 기능을 추가했습니다.
...
프롬프트 옵션(토글)을 채팅별로 독립 저장하고, 프리셋마다 토글 값을 따로 관리합니다.
특정 채팅에 프리셋을 바인딩하면 해당 채팅에 들어갈 때 자동으로 프리셋이 적용됩니다.
채팅을 전환해도 각각의 설정이 그대로 유지되어, 캐릭터별로 다른 프롬프트를 쓸 때 편리합니다.
